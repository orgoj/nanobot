...
class TestRoutingIntegration:
    ...
    async def test_full_routing_pipeline(self, tmp_path):
        ...
        config = RoutingConfig(
            ...
            tiers={
                ...
                "coding": {"model": "kimi-k2.5", "cost_per_mtok": 0.45},
            }
        )
    ...
    async def test_complex_message_routing(self, tmp_path):
        ...
        mock_msg = Mock()
        mock_msg.content = "Debug this distributed system with race conditions and memory leaks"
    ...
    async def test_sticky_routing_across_messages(self, tmp_path):
        ...
        # Sticky routing should maintain tier (or allow downgrade if appropriate)
    ...
    def test_end_to_end_classification(self):
        ...
        test_cases = [
            ("How are you today?", RoutingTier.SIMPLE),
            ("Write a Python function to parse JSON", RoutingTier.CODING),
            ("Debug this race condition in my distributed system", RoutingTier.COMPLEX),
            ("Prove this mathematical theorem step by step", RoutingTier.REASONING),
        ]

        for message, expected_tier in test_cases:
            decision, scores = classify_content(message)
            if expected_tier == RoutingTier.CODING:
                # CODING might fall back to MEDIUM or vice versa depending on exact matches
                assert decision.tier in [RoutingTier.CODING, RoutingTier.MEDIUM, RoutingTier.COMPLEX], f"Failed for: {message}"
            else:
...
class TestRoutingWithLLMFallback:
    ...
    async def test_client_side_classification_sufficient(self):
        ...
        # Trie match should have high confidence
        decision, scores = classifier.classify("Thanks")
    ...
    async def test_llm_fallback_for_uncertain(self):
        ...
        classifier = ClientSideClassifier(min_confidence=0.99)  # Impossible to reach
        ...
        decision = await sticky_router.classify("Some complex query that won't match exactly", mock_session)
