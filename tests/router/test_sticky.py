...
class TestStickyRouter:
    ...
    async def test_classify_simple_no_history(self):
        ...
        session = MockSession()
        # Use a message that is SIMPLE but NOT an "always_simple" bypass (confidence < 0.9)
        decision = await router.classify("I have a quick question about something entirely unrelated", session)
    ...
    async def test_sticky_maintains_complex_tier(self):
        ...
        # Set up existing complex history
        session.messages = [
        ...
        # Current message that would normally be simple but isn't an always-simple bypass
        decision = await router.classify("Can you help me understand the implications of this approach?", session)
    ...
    async def test_downgrade_when_explicitly_simple(self):
        ...
        # Simple message with downgrade markers
    ...
    async def test_with_llm_fallback(self):
        ...
        classifier = ClientSideClassifier(min_confidence=0.99)  # Force fallback
        ...
        session = MockSession()
        # Use a query that won't have 0.99 confidence client-side
        decision = await router.classify("Tell me a very detailed story about space travel", session)
    ...
    async def test_feedback_recorded(self):
        ...
        session = MockSession()
        decision = await router.classify("Generic question that needs LLM review", session)
...
class TestStickyRoutingEdgeCases:
    ...
    async def test_session_metadata_updated(self):
        ...
        session = MockSession()
        # Use message that isn't always-simple bypass
        await router.classify("What is the difference between Python and Java?", session)
