...
class TestClientSideClassifier:
    ...
    def test_classify_reasoning_query(self):
        ...
        classifier = ClientSideClassifier()
        # Using a more comprehensive reasoning query
        decision, scores = classifier.classify(
            "Prove this mathematical theorem step by step and walk me through the logic"
        )

        assert decision.tier == RoutingTier.REASONING
        assert scores.reasoning_markers > 0.4
    ...
    def test_classify_returns_metadata(self):
        ...
        classifier = ClientSideClassifier()
        decision, scores = classifier.classify("Some random message")
    ...
    def test_determine_tier_thresholds(self):
        ...
        # Use content that won't match any explicit pattern
        msg = "A generic message that doesn't match any specific pattern"
        
        # Test each tier
        tier, _ = classifier._determine_tier(0.98, msg, context, scores)
        assert tier == RoutingTier.REASONING
        tier, _ = classifier._determine_tier(0.90, msg, context, scores)
        assert tier == RoutingTier.COMPLEX
        tier, _ = classifier._determine_tier(0.70, msg, context, scores)
        assert tier == RoutingTier.MEDIUM
        tier, _ = classifier._determine_tier(0.30, msg, context, scores)
    ...
    def test_determine_tier_reasoning_override(self):
        ...
        classifier = ClientSideClassifier()
        context = ClassificationContext()
        scores = ClassificationScores(reasoning_markers=0.8)

        content = "Prove step by step this theorem"
        tier, _ = classifier._determine_tier(0.96, content, context, scores)
        assert tier == RoutingTier.REASONING
    ...
    def test_calculate_scores_reasoning_markers(self):
        ...
        scores = classifier._calculate_scores("Prove this theorem step by step", context)
        assert scores.reasoning_markers > 0.4
    ...
    def test_calculate_scores_code_presence(self):
        ...
        scores = classifier._calculate_scores("def function(): pass", context)
        assert scores.code_presence > 0.3

        # Code blocks boost score
        context_with_blocks = ClassificationContext(has_code_blocks=True)
    ...
    def test_calculate_scores_token_count(self):
        ...
        short = classifier._calculate_scores("Hi", context)
        long = classifier._calculate_scores(" ".join(["word"] * 1000), context)
