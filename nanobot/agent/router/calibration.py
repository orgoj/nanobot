...
class CalibrationManager:
    ...
    def _analyze_accuracy(self) -> dict:
        ...
        for tier in ["simple", "medium", "complex", "coding", "reasoning"]:
            tier_records = [
                r
                for r in self._classifications
    ...
    def _calibrate_thresholds(self) -> dict:
        ...
        for tier in current_thresholds.keys():
            ...
            for threshold in test_thresholds:
                ...
                above_threshold = [
                    r for bucket, records in buckets.items() if bucket >= threshold for r in records
                ]
                ...
                if weighted_score > (
                    best_accuracy * 0.8 + (min(best_sample_size, 100) / 100) * 0.2
                ):
            ...
            if best_threshold != current_thresholds[tier]:
                baseline_accuracy = sum(
                    1 for r in tier_records if r["client_tier"] == r["llm_tier"]
                ) / len(tier_records)
                adjustments[tier] = {
    ...
    def get_calibration_report(self) -> dict:
        ...
        top_patterns = sorted(patterns, key=lambda p: p.effectiveness_score, reverse=True)[:10]

        # Low performing patterns (candidates for eviction)
        low_patterns = [p for p in patterns if not p.is_effective and p.times_used > 10]
