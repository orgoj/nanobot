...
class LLMRouter:
    ...
    async def classify(
        self, content: str, context: Optional[ClassificationContext] = None
    ) -> RoutingDecision:
        """
    ...
    def _build_prompt(self, content: str, context: Optional[ClassificationContext]) -> str:
        ...
        if context:
            ...
            if context.has_negations and context.negation_details:
                negations_str = ", ".join(
                    [n.get("negation", "unknown") for n in context.negation_details[:3]]
                )
                context_hints.append(
            ...
            if context.urgency:
                ...
                context_hints.append(
                    f"\nContext: Urgency detected: '{urgency_str}'. User needs quick response."
                )
    ...
    def _validate_with_context(self, result: dict, context: ClassificationContext) -> dict:
        """
    ...
    async def _call_llm(self, messages: list[dict[str, Any]], model: str | None = None) -> str:
        ...
        # Wait with timeout
        try:
            response = await asyncio.wait_for(llm_task, timeout=self.timeout_ms / 1000.0)
