...
class ClientSideClassifier:
    ...
    def _load_patterns(self) -> None:
        ...
        if self.patterns_file and self.patterns_file.exists():
            try:
                ...
                self._patterns = [RoutingPattern.from_dict(p) for p in data.get("patterns", [])]
            except Exception:
    ...
    def classify(self, content: str) -> Tuple[RoutingDecision, ClassificationScores]:
        ...
        tier, pattern_confidence = self._determine_tier(
            calculated_confidence, content, context, scores
        )
    ...
    def _detect_action_type(self, content: str) -> str:
        ...
        if re.search(
            r"\b(write|create|generate|build|implement|make|develop|code|script)\b", content
        ):
            return "write"

        # Explaining/Describing actions
        if re.search(
            r"\b(explain|describe|tell me about|what is|how does|why|clarify|elaborate)\b", content
        ):
            return "explain"
        ...
        if re.search(
            r"\b(fix|repair|correct|improve|optimize|refactor|enhance|upgrade|update)\b", content
        ):
            return "fix"
                ...
                for ending in clause_endings:
                    ...
                    if end_pos != -1 and end_pos < scope_end:
                        ...
                        if ending == ",":
                            ...
                            if any(
                                word in next_part for word in ["instead", "just", "only", "but"]
                            ):
                                scope_end = end_pos
                        else:
                            scope_end = end_pos
    ...
    def _calculate_scores(
        self, content: str, context: ClassificationContext
    ...
    def _score_patterns(
        self, content: str, patterns: List[str], context: ClassificationContext = None
    ) -> float:
        """
        ...
        for pattern in patterns:
            ...
            if pattern_lower in content_lower:
                ...
                if is_negated:
                    ...
                    is_domain_indicator = any(
                        domain_word in pattern_lower
                        for domain_word in [
                            "code",
                            "function",
                            "git",
    ...
    def _determine_tier(
                ...
                if pattern.confidence >= 0.90:
                    ...
                    if pattern.tier == RoutingTier.CODING:
                        if context.action_type == "explain":
                            ...
                            for neg in context.negations:
                                if any(
                                    word in neg["scope_text"]
                                    for word in ["write", "create", "build", "make"]
                                ):
                                    return RoutingTier.MEDIUM, pattern.confidence
        ...
        reasoning_count = sum(
            1
            for word in ["prove", "theorem", "step by step", "formal proof"]
        ...
        if confidence >= self.thresholds["reasoning"]:
            ...
            if (
                ...
                or scores.multi_step_patterns > 0.4
                or scores.imperative_verbs > 0.6
                or scores.code_presence > 0.4
            ):
                return RoutingTier.MEDIUM, 0.0
