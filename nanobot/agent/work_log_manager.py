...
from nanobot.agent.work_log import LogLevel, WorkLog, WorkLogEntry
...
class WorkLogManager:
    ...
    def start_session(self, session_id: str, query: str) -> WorkLog:
        ...
        self.current_log = WorkLog(session_id=session_id, query=query, start_time=datetime.now())
    ...
    def log_tool(
        self, tool_name: str, tool_input: dict, tool_output: any, tool_status: str, duration_ms: int
    ) -> Optional[WorkLogEntry]:
    ...
    def get_last_log(self) -> Optional[WorkLog]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor = conn.execute("SELECT * FROM work_logs ORDER BY start_time DESC LIMIT 1")
                row = cursor.fetchone()
    ...
    def get_log_by_session(self, session_id: str) -> Optional[WorkLog]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.execute("SELECT * FROM work_logs WHERE session_id = ?", (session_id,))
                row = cursor.fetchone()
        ...
        for entry_row in entries_cursor:
            log.entries.append(
                WorkLogEntry(
                    ...
                    details=json.loads(entry_row["details_json"])
                    if entry_row["details_json"]
                    else {},
                    confidence=entry_row["confidence"],
                    duration_ms=entry_row["duration_ms"],
                    tool_name=entry_row["tool_name"],
                    tool_input=json.loads(entry_row["tool_input_json"])
                    if entry_row["tool_input_json"]
                    else None,
                    tool_output=json.loads(entry_row["tool_output_json"])
                    if entry_row["tool_output_json"]
                    else None,
