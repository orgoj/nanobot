...
class SkillSecurityScanner:
    ...
    CRITICAL_PATTERNS = {
        ...
        "credential_exfiltration": (
            re.compile(
                r"curl.*-d.*key|wget.*--post-data.*secret|base64\s+.*\|\s*curl", re.IGNORECASE
            ),
    ...
    HIGH_PATTERNS = {
        "curl_pipe_bash": (
            re.compile(
                r"curl\s+[^|]*\|\s*(bash|sh|zsh)|wget\s+[^|]*\|\s*(bash|sh|zsh)", re.IGNORECASE
            ),
        ...
        "persistence_mechanism": (
            re.compile(
                r"LaunchAgents|LaunchDaemons|crontab|cron\.|\.bashrc|\.zshrc|\.bash_profile",
                re.IGNORECASE,
            ),
    ...
    MEDIUM_PATTERNS = {
        "base64_decoding": (
            re.compile(
                r"base64\s+-d|base64\s+--decode|atob\(|Buffer\.from\([^,]*base64", re.IGNORECASE
            ),
    ...
    def scan_skill(self, skill_path: Path) -> SecurityReport:
        ...
        report = SecurityReport(skill_name=skill_name, findings=findings, passed=passed)

        logger.info(
            f"Security scan for {skill_name}: {len(findings)} findings, score {report.total_risk_score}/100"
        )
...
def format_report_for_cli(report: SecurityReport) -> str:
    ...
    lines.append(
        f"{risk_color}║[/] Risk Score: {report.total_risk_score}/100{' ' * 40} {risk_color}║[/]"
    )
    lines.append(
        f"{risk_color}║[/] Critical: {report.critical_count} | High: {report.high_count}{' ' * 29} {risk_color}║[/]"
    )
    lines.append(f"{risk_color}╚═══════════════════════════════════════════════════════════╝[/]")
    ...
    if not report.passed:
        ...
        lines.append(
            "[dim]Review the findings above or use --ignore-security to force install (not recommended)[/]"
        )
    else:
