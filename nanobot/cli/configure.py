...
def _show_status(summary: dict):
    ...
    # Providers
    configured_providers = [name for name, info in summary["providers"].items() if info["has_key"]]
    if configured_providers:
        table.add_row("LLM Providers", f"[green]✓[/green] {', '.join(configured_providers)}")
    else:
        table.add_row("LLM Providers", "[red]✗[/red] None configured [dim](Required)[/dim]")

    # Channels
    enabled_channels = [
        name for name, info in summary["channels"].items() if info.get("enabled", False)
    ]
    if enabled_channels:
        table.add_row("Channels", f"[green]✓[/green] {', '.join(enabled_channels)}")
    else:
        table.add_row("Channels", "[dim]○ None enabled (Optional)[/dim]")
...
def _get_channels_status(summary: dict) -> str:
    ...
    enabled_channels = [
        name for name, info in summary.get("channels", {}).items() if info.get("enabled", False)
    ]
...
def _get_agents_status(summary: dict) -> str:
    ...
    has_any_provider = any(p.get("has_key", False) for p in summary.get("providers", {}).values())
...
def _show_main_menu(summary: dict) -> str:
    ...
    console.print(
        Panel("[bold]Main Menu[/bold]\nSelect a category to configure:", border_style="blue")
    )
    ...
    if not has_required:
        ...
        options.append(
            ("1", "providers", f"🤖 Model Providers {providers_status} [red](Required)[/red]")
        )
    else:
    ...
    while True:
        choice = Prompt.ask("Select option", choices=list(choice_map.keys()))
...
def _configure_providers(summary: dict):
    ...
    console.print(
        Panel("[bold]Model Providers[/bold]\nSelect a provider to configure:", border_style="blue")
    )
    ...
    # Get choice
    choice = Prompt.ask(
        "Select provider", choices=[str(i) for i in range(len(providers_list) + 1)], default="1"
    )
...
def _configure_single_provider(name: str, schema: dict):
    ...
    console.print(Panel(f"[bold]{name.title()} Configuration[/bold]", border_style="blue"))
    ...
    with console.status(f"[cyan]Saving {name} configuration...[/cyan]", spinner="dots"):
        result = asyncio.run(tool.execute(path=f"providers.{name}.apiKey", value=api_key))
        ...
        if action[0] == "toggle":
            ...
            if new_value:
                ...
                if "Error" not in result:
                    ...
                    console.print("[dim]Start the gateway to activate: nanobot gateway[/dim]")
            ...
            if field_name == "token":
                ...
                prompt_text = (
                    "Enter allowed user IDs/usernames (comma-separated, leave empty for all users)"
                )
            elif field_name == "bridgeUrl":
            ...
            if value:
                result = asyncio.run(
                    tool.execute(path=f"channels.{name}.{field_name}", value=value)
                )
                if "Error" in result:
                    console.print(f"[red]{result}[/red]")
                else:
    ...
    if Confirm.ask("Enable voice transcription?", default=True):
        ...
        if api_key:
            with console.status("[cyan]Saving Groq configuration...[/cyan]", spinner="dots"):
                result = asyncio.run(tool.execute(path="providers.groq.apiKey", value=api_key))
            if "Error" not in result:
            ...
            console.print(
                "[yellow]⚠ No API key provided. Voice messages won't be transcribed.[/yellow]"
            )
...
def _configure_agents():
    ...
    console.print(Panel("[bold]Agent Settings[/bold]", border_style="blue"))
...
def _configure_routing():
    ...
    if is_enabled:
        ...
        for tier_name, tier_config, use_case in tiers_info:
            tiers_table.add_row(
                tier_name, tier_config.model, f"${tier_config.cost_per_mtok:.2f}", use_case
            )
        ...
        if choice == "1":
            ...
            tier_choice = Prompt.ask(
                "Select tier", choices=["0", "1", "2", "3", "4", "5"], default="0"
            )
            ...
            if tier_choice != "0":
                ...
                new_model = Prompt.ask(
                    "Enter model name (or press Enter to keep current)", default=""
                )
                if new_model:
                    # Validate the model
                    ...
                    if proceed:
                        result = asyncio.run(
                            tool.execute(
                                path=f"routing.tiers.{selected_tier}.model", value=new_model
                            )
                        )
                        console.print(f"[green]{result}[/green]")
                    else:
            ...
            console.print(
                f"Current client classifier confidence: {config.routing.client_classifier.min_confidence}"
            )

            new_confidence = Prompt.ask(
                "Enter new confidence threshold (0.0-1.0, or press Enter to keep)", default=""
            )
            if new_confidence:
                result = asyncio.run(
                    tool.execute(
                        path="routing.clientClassifier.minConfidence", value=float(new_confidence)
                    )
                )
                console.print(f"[green]{result}[/green]")
            ...
            console.print(
                "This model is used to classify ambiguous queries when the client-side classifier is unsure."
            )
            console.print(f"Current: [cyan]{config.routing.llm_classifier.model}[/cyan]")

            new_model = Prompt.ask(
                "Enter new classifier model (or press Enter to keep)", default=""
            )
            if new_model:
                # Validate the model
                validation = tool.validate_model_for_routing(new_model)
                ...
                if proceed:
                    result = asyncio.run(
                        tool.execute(path="routing.llmClassifier.model", value=new_model)
                    )
                    console.print(f"[green]{result}[/green]")
                else:
                    console.print("[dim]Cancelled.[/dim]")
...
def _configure_tools():
    ...
    while True:
        console.print(Panel("[bold]Tool Settings[/bold]", border_style="blue"))
        ...
        if choice == "1":
            ...
            while True:
                ...
                console.print(Panel("[bold]Security Settings[/bold]", border_style="blue"))
                ...
                if menu_choice == "1":
                    ...
                    if Confirm.ask(
                        ...
                        with console.status(
                            "[cyan]Updating security settings...[/cyan]", spinner="dots"
                        ):
                            result = asyncio.run(
                                tool.execute(path="tools.evolutionary", value=new_value)
                            )
                    ...
                    if new_path:
                        ...
                        if new_path not in current_paths:
                            ...
                            with console.status("[cyan]Adding path...[/cyan]", spinner="dots"):
                                asyncio.run(
                                    tool.execute(path="tools.allowedPaths", value=updated_paths)
                                )
                    ...
                    rm_idx = Prompt.ask(
                        "Select number",
                        choices=["0"]
                        + [str(i) for i in range(1, len(config.tools.allowed_paths) + 1)],
                        default="0",
                    )
                    if rm_idx == "0":
                        continue
                    ...
                    with console.status("[cyan]Removing path...[/cyan]", spinner="dots"):
                        asyncio.run(tool.execute(path="tools.allowedPaths", value=updated_paths))
                    ...
                    if new_path:
                        ...
                        if new_path not in current_paths:
                            updated_paths = current_paths + [new_path]
                            with console.status(
                                "[cyan]Adding protected path...[/cyan]", spinner="dots"
                            ):
                                asyncio.run(
                                    tool.execute(path="tools.protectedPaths", value=updated_paths)
                                )
                    ...
                    rm_idx = Prompt.ask(
                        "Select number",
                        choices=["0"]
                        + [str(i) for i in range(1, len(config.tools.protected_paths) + 1)],
                        default="0",
                    )
                    if rm_idx == "0":
                        continue
                    ...
                    with console.status("[cyan]Removing protected path...[/cyan]", spinner="dots"):
                        asyncio.run(tool.execute(path="tools.protectedPaths", value=updated_paths))
            ...
            if choice == "1":
                ...
                if api_key:
                    with console.status(
                        "[cyan]Saving web search configuration...[/cyan]", spinner="dots"
                    ):
                        result = asyncio.run(
                            tool.execute(path="tools.web.search.apiKey", value=api_key)
                        )
                    if "Error" not in result:
                        console.print(f"[green]{result}[/green]")
...
def _configure_gateway():
    ...
    while True:
        ...
        console.print(Panel("[bold]Gateway Configuration[/bold]", border_style="blue"))
        ...
        if choice == "0":
            ...
            new_host = Prompt.ask(
                "Enter new host (e.g., 0.0.0.0, 127.0.0.1, localhost)", default=config.gateway.host
            )
            if new_host and new_host != config.gateway.host:
                with console.status("[cyan]Updating host...[/cyan]", spinner="dots"):
                    result = asyncio.run(tool.execute(path="gateway.host", value=new_host))
                console.print(f"[green]{result}[/green]")
                ...
                try:
                    ...
                    if port_int != config.gateway.port:
                        with console.status("[cyan]Updating port...[/cyan]", spinner="dots"):
                            result = asyncio.run(tool.execute(path="gateway.port", value=port_int))
                        console.print(f"[green]{result}[/green]")
                except ValueError:
...
def _show_detailed_status():
    ...
    console.print(Panel("[bold]Detailed Configuration Status[/bold]", border_style="blue"))

    # Config file location
    console.print(f"\n[dim]Config file:[/dim] {config_path}")
    console.print(
        f"[dim]Exists:[/dim] {'[green]Yes[/green]' if config_path.exists() else '[red]No[/red]'}\n"
    )
    ...
    if config.routing.enabled:
        console.print(
            f"  Default Model: [dim]{config.agents.defaults.model}[/dim] [yellow](overridden by Smart Routing)[/yellow]"
        )
    else:
    ...
    for provider_name in all_providers:
        ...
        table.add_row(
            provider_name.title(), "[green]✓[/green]" if has_key else "[dim]✗[/dim]", status
        )
    ...
    if config.routing.enabled:
        ...
        for tier_name, tier_config in tiers:
            tiers_table.add_row(tier_name, tier_config.model, f"${tier_config.cost_per_mtok:.2f}")
    ...
    has_web_search = bool(config.tools.web.search.api_key)
    console.print(
        f"  Web Search: {'[green]Configured[/green]' if has_web_search else '[dim]Not configured[/dim]'}"
    )
    if has_web_search:
