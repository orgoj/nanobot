...
class PreferencesAggregator:
    ...
    async def aggregate_preferences(self) -> SummaryNode:
        ...
        high_conf = [learning for learning in learnings if learning.confidence >= 0.8]
        if high_conf:
        ...
        # Add recommendations
        recommendations = [
            learning.recommendation for learning in learnings if learning.recommendation
        ]
        if recommendations:
        ...
        tool_prefs = [learning for learning in learnings if learning.tool_name]
        if tool_prefs:
    ...
    def get_high_confidence_preferences(self, min_confidence: float = 0.8) -> list[Learning]:
        ...
        """
        learnings = self.store.get_all_learnings(active_only=True)
        return [learning for learning in learnings if learning.confidence >= min_confidence]
