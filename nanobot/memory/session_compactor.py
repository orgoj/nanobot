...
class SummaryCompactionMode:
    ...
    async def compact(
        self, messages: list[dict[str, Any]], target_tokens: int, preserve_recent: int = 20
    ) -> tuple[list[dict[str, Any]], dict[str, Any]]:
        """
        ...
        if recent_tokens >= target_tokens:
            logger.warning(
                f"Recent messages ({recent_tokens} tokens) exceed target ({target_tokens})"
            )
    ...
    async def _summarize_chunk(self, messages: list[dict[str, Any]]) -> str:
        ...
        return (
            f"Conversation segment: {user_msgs} user messages, {assistant_msgs} assistant responses"
        )
...
class TokenLimitCompactionMode:
    ...
    async def compact(
        self, messages: list[dict[str, Any]], target_tokens: int, min_messages: int = 10
    ) -> tuple[list[dict[str, Any]], dict[str, Any]]:
        """
...
class SessionCompactor:
    ...
    async def compact_session(
        self, session: Session, max_tokens: int | None = None
    ) -> CompactionResult:
        """
        ...
        if mode == "summary":
            compacted, stats = await mode_impl.compact(
                messages, target_tokens, preserve_recent=self.config.preserve_recent
            )
        else:  # token-limit
            compacted, stats = await mode_impl.compact(
                messages, target_tokens, min_messages=self.config.min_messages
            )
