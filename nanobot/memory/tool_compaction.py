...
class ToolOutputStore:
    ...
    def get_output(self, output_id: str) -> ToolOutputEntry | None:
        ...
        conn.execute(
            "UPDATE tool_outputs SET accessed_count = accessed_count + 1 WHERE id = ?", (output_id,)
        )
        conn.commit()
    ...
    def cleanup_old_outputs(self, max_age_hours: int = 24) -> int:
        ...
        conn = self.memory_store._get_connection()
        cursor = conn.execute("DELETE FROM tool_outputs WHERE created_at < ?", (cutoff,))
        conn.commit()
...
class ToolOutputCompactor:
            ...
            if msg.get("role") == "tool" or self._is_tool_result(msg):
                ...
                if len(content) > self.max_context_chars:
                    ...
                    result = self.process_tool_result(
                        tool_name=tool_name, result=content, session_key=session_key
                    )
    ...
    def detect_redundant_calls(self, messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """
    ...
    def _hash_tool_calls(self, tool_uses: list[dict]) -> str:
        ...
        tool_str = "|".join([f"{t.get('name', '')}:{str(t.get('input', {}))}" for t in tool_uses])
...
def compact_tool_outputs(
    ...
    Returns:
        Messages with compacted tool outputs.
    """
    compactor = ToolOutputCompactor(memory_store=memory_store, max_context_chars=max_context_chars)
