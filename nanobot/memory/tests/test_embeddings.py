...
class TestEmbeddingProvider:
    ...
    def test_api_fallback(self):
        ...
        config = EmbeddingConfig(provider="local", api_fallback=True, lazy_load=True)
        provider = EmbeddingProvider(config)
...
class TestSemanticSearch:
    ...
    def test_search_events(self, temp_db):
        ...
        results = store.search_events(query_embedding=query_embedding, limit=3, threshold=0.5)
    ...
    def test_search_with_threshold(self, temp_db):
        ...
        results_high = store.search_events(query_embedding=query_embedding, threshold=0.9)

        # Low threshold should return more results
        results_low = store.search_events(query_embedding=query_embedding, threshold=0.1)
    ...
    def test_search_by_session(self, temp_db):
        ...
        results = store.search_events(
            query_embedding=query_embedding, session_key="discord:123", limit=10
        )
